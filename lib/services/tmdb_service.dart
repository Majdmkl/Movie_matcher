...
class TMDBService {
  ...
  Future<Movie?> getMovieById(int movieId) async {
    ...
    try {
      final response = await http.get(url).timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
        return _parseMovieDetails(json.decode(response.body));
      }
      return null;
    } catch (e) {
  ...
  Future<Movie?> getTVShowById(int tvId) async {
    ...
    try {
      final response = await http.get(url).timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
        return _parseTVShowDetails(json.decode(response.body));
      }
      return null;
    } catch (e) {
  ...
  Movie _parseMovie(Map<String, dynamic> json) {
    ...
    final genres = genreIds.map((id) => _movieGenres[id]).where((n) => n != null).cast<String>().toList();
  ...
  Movie _parseTVShow(Map<String, dynamic> json) {
    final genreIds = (json['genre_ids'] as List?)?.cast<int>() ?? [];
    final genres = genreIds.map((id) => _tvGenres[id]).where((n) => n != null).cast<String>().toList();
